...
static StackMap *search_stack(char *stack, size_t len)
{
	char *p;
	...
	while (!result && fgets(buffer, 1024, maps)) {
		...
		*p = '\0';
		...
		log("map 0x%lx -> 0x%lx size: %dk from '%s' '%s'\n",
		    (long)start, (long)end, (int)(end - start) / 1024, buffer, p + 1);
 ...
 * cleanup after it).
 */
static int close_pid(DumpState *s, int avoid_kill)
{
	int pid;
 ...
 */
int buffers_extract_and_dump(const char *output_path, Arguments *remote_args)
{
	int i, pid, ret = 0;
	DumpState *state;
...
int dump_header(const char *output_path)
{
...

	{
		...
		if (!gethostname(host_buf, 2047) && !getdomainname(domain_buf, 2048)) {
