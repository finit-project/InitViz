...
static inline PidEntry *get_pid_entry(pid_t pid)
{
 ...
 */
static struct taskstats *get_taskstats(pid_t pid)
{
	struct taskstats *ts;
 ...
 * in the kernel - we have to manually aggregate here.
 */
static struct taskstats *get_tgid_taskstats(PidScanner *scanner)
{
	pid_t tpid;
	struct taskstats *ts;
 ...
 */
static void dump_taskstat(BufferFile *file, PidScanner *scanner)
{
	pid_t ppid;
	int output_len;
	...
	output_len = snprintf(output_line, 1024, "%d %d %s %lld %lld %lld\n",
			      ...
			      (long long)ts->blkio_delay_total, (long long)ts->swapin_delay_total);
	if (output_len < 0)
		return;
...
static void dump_proc_stat(BufferFile *file, int pid)
{
	int fd;
...
static void dump_cmdline(BufferFile *file, pid_t pid)
{
	int fd, len;
...
/* ---- called from netlink thread ---- */
static void dump_paternity(BufferFile *file, pid_t pid, pid_t ppid)
{
	char str[1024];
	/* <Child> <Parent> */
...
/* ---- called from netlink thread ---- */
static void pid_event_cb(const PidScanEvent *event, void *user_data)
{
...
static int have_dev_tmpfs(void)
{
	FILE *mi;
	int ret = 0;
	char buffer[4096];
	...
	while (fgets(buffer, 4096, mi)) {
		...
		if (strstr(buffer, "/dev") && strstr(buffer, "rw") && strstr(buffer, "tmpfs")) {
			ret = 1;
			break;
		}
	}
 ...
 */
static int sanity_check_initrd(void)
{
	FILE *cmdline;
	char buffer[4096];
	...
	if (!strstr(buffer, "init=") || !strstr(buffer, "bootchartd")) {
 ...
 * running system...
 */
static int chroot_into_dev(void)
{
...
static void usage(void)
{
	...
	fprintf(stderr,
		"   --dump <path>	if another bootchart is running, dumps it's state to <path> and exits.\n");
...
static int enter_environment(int console_debug)
{
	...
	if (mount("none", PROC_PATH, "proc", MS_NODEV | MS_NOEXEC | MS_NOSUID, NULL) < 0) {
		if (errno != EBUSY) {
...
static int clean_enviroment(void)
{
	int ret = 0;
...
static void setup_sigaction(int sig)
{
...
/*		log ("arg %d = '%s'\n", i, argv[i]); */
		...
		else if (!strcmp(argv[i], "-c") || !strcmp(argv[i], "--console"))
			args->console_debug = 1;
