...
static int netlink_pid_scanner_free(PidScanner *scanner)
{
...
static int find_pid_an_idx(NetLinkPidScanner *nls, pid_t pid, int create)
{
	int i, j;
...
static void insert_pid(NetLinkPidScanner *nls, pid_t pid, pid_t parent)
{
	int idx = find_pid_an_idx(nls, pid, 1);
	nls->procs[idx]->parent = parent;
}

static void insert_pid_thread(NetLinkPidScanner *nls, pid_t pid, pid_t thread)
{
	int n, idx;
	Process *p;
...
static void remove_pid(NetLinkPidScanner *nls, pid_t pid)
{
	int i;
...
static void remove_pid_thread(NetLinkPidScanner *nls, pid_t pid, pid_t thread)
{
	int i, j;
	Process *p;
...
static void netlink_pid_scanner_restart(PidScanner *scanner)
{
	int ev_count, i;
...
static void netlink_pid_scanner_bootstrap(NetLinkPidScanner *nls)
{
	int pid;
...
static pid_t netlink_pid_scanner_next(PidScanner *scanner)
{
	NetLinkPidScanner *nls = (NetLinkPidScanner *) scanner;
...
static pid_t netlink_pid_scanner_get_cur_pid(PidScanner *scanner)
{
	NetLinkPidScanner *nls = (NetLinkPidScanner *) scanner;
	if (nls->cur_proc >= nls->n_procs)
		return 0;
...
static pid_t netlink_pid_scanner_get_cur_ppid(PidScanner *scanner)
{
	NetLinkPidScanner *nls = (NetLinkPidScanner *) scanner;
	if (nls->cur_proc >= nls->n_procs)
		return 0;
...
static void netlink_pid_scanner_get_tasks_start(PidScanner *scanner)
{
	NetLinkPidScanner *nls = (NetLinkPidScanner *) scanner;
	nls->cur_thread = 0;
}

static pid_t netlink_pid_scanner_get_tasks_next(PidScanner *scanner)
{
	NetLinkPidScanner *nls = (NetLinkPidScanner *) scanner;
	if (nls->cur_proc >= nls->n_procs || !nls->procs[nls->cur_proc]->threads[nls->cur_thread])
		return 0;
	return nls->procs[nls->cur_proc]->threads[nls->cur_thread++];
}

static void netlink_pid_scanner_get_tasks_stop(PidScanner *scanner)
{
	NetLinkPidScanner *nls = (NetLinkPidScanner *) scanner;
	nls->cur_thread = 0;
}

static void handle_news(NetLinkPidScanner *nls, struct cn_msg *cn_hdr)
{
	...
	case PROC_EVENT_FORK:
		...
		if (ev->event_data.fork.child_pid == ev->event_data.fork.child_tgid)
			pid_scanner_emit_paternity((PidScanner *) nls,
						   ev->event_data.fork.child_tgid, ev->event_data.fork.parent_tgid);
		/* drop through */
	case PROC_EVENT_EXIT:
		pthread_mutex_lock(&nls->lock);
...
static size_t netlink_recvfrom(NetLinkPidScanner *nls, char *buffer)
{
	...
	return recvfrom(nls->socket, buffer, BUFF_SIZE, 0, (struct sockaddr *)&from_nla, &from_nla_len);
}
...
PidScanner *pid_scanner_new_netlink(PidScanEventFn event_fn, void *user_data)
{
	char buff[BUFF_SIZE];
	struct cn_msg *cn_hdr;
	...
	nls = (NetLinkPidScanner *) pid_scanner_alloc(sizeof(NetLinkPidScanner), event_fn, user_data);
...
#undef INIT
	...
	*mcop_msg = PROC_CN_MCAST_LISTEN;
	...
	if (send(nls->socket, nl_hdr, nl_hdr->nlmsg_len, 0) != nl_hdr->nlmsg_len) {
		...
		ZERO_ARRAY(buff);
		recv_len = netlink_recvfrom(nls, buff);
		if (recv_len < 1 || !NLMSG_OK(nlh, recv_len) || nlh->nlmsg_type != NLMSG_DONE) {
